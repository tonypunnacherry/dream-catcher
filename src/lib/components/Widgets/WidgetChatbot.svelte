<script lang="ts">
  ...
  let id: string | undefined = widget?.id;
  let widgetIndex: number = AppState.getWidgetByTitle(title).index || -1;
  let model: string | undefined =
    $AppState.widgets[widgetIndex].parameters?.model;
  let initialUserMessage: string | undefined =
    $AppState.widgets[widgetIndex].initialUserMessage + "";
  let initialAssistantMessage: string | undefined =
    $AppState.widgets[widgetIndex].initialAssistantMessage + "";
  let placeholder: string | undefined =
    $AppState.widgets[widgetIndex].placeholder;
  let placeholderText = prepareGeneratingPlaceholder(
    initialUserMessage || "",
    "chat",
  );
  ...
  $: {
    ...
    valuesChanged = !(
      JSON.stringify(dependentValues) === JSON.stringify(rememberValues)
    );
    ...
    if (valuesValid && valuesChanged) {
      ...
      let currentInitialUserMessage = processPlaceholders(
        initialUserMessage || "",
        $AppState,
      );
      let currentInitialAssistantMessage = processPlaceholders(
...
<div class="party-widget-ai" {id}>
  ...
  <div class="party-widget-contents flex flex-col">
    {#if (messages?.length || 0) > 0}
      <div
        class="flex flex-col-reverse min-h-48 max-h-72 overflow-auto"
    ...
    {:else}
      <span class="party-widget-placeholder min-h-48"
        >{@html placeholderText}</span
      >
    {/if}
    <form
      class="border-t-2 border-black mt-2 pt-2 flex mt-auto"
      on:submit|preventDefault={addMessage}
    >
      <input
        type="text"
        {placeholder}
        class="flex-grow flex-shrink bg-inherit outline-none"
        style="font-family:'Poppins', sans-serif;"
        bind:value={newMessage}
        disabled={loadingStatus}
      />
      <button type="submit" class="flex-shrink-0 pl-2" style="margin-left:5px;" disabled={loadingStatus}
        ><span class="material-symbols-outlined" style="position: relative;left:5px;top:2px;"> send </span></button
      >
    </form>
  </div>
</div>
